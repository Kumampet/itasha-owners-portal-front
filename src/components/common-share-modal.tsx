...
export function ShareModal({
    ...
    // デフォルトはすべて有効
    const enableX = config.enableX !== false;
    const enableInstagram = config.enableInstagram !== false;
    const enableLine = config.enableLine !== false;
    const enableWebShare = config.enableWebShare !== false;

    const encodedUrl = encodeURIComponent(config.itemUrl);

    // X（Twitter）でシェア
    const handleShareX = () => {
        const tweetText = `${config.tweetPrefix}\n${config.itemName}\n${config.itemUrl}`;
    ...
    const handleShareInstagram = async () => {
        ...
        const shareText = `${config.itemName}\n${config.itemUrl}`;
        try {
            await navigator.clipboard.writeText(shareText);
            alert(config.instagramCopyMessage);
        } catch (error) {
    ...
    const handleCopyUrl = async () => {
        try {
            await navigator.clipboard.writeText(config.itemUrl);
