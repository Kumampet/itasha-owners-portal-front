    ...
    <div className="w-full px-4 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-border bg-card p-8 text-center">
          <p className="text-sm text-muted-foreground">お問い合わせがありません</p>
        </div>
      ) : (
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className={`flex-1 ${contact.status === "RESOLVED" ? "text-zinc-400" : ""}`}>
                  <div className="mb-2 flex items-center gap-2">
                    ...
                    <span className={`text-xs ${contact.status === "RESOLVED" ? "text-zinc-400" : "text-muted"}`}>
                      {contact.submitter?.email || contact.email}
                    </span>
                  </div>
                  <h3 className={`text-base font-semibold ${contact.status === "RESOLVED" ? "text-zinc-400" : "text-foreground"}`}>
                    {contact.title}
                  </h3>
                  <p className={`mt-1 text-sm ${contact.status === "RESOLVED" ? "text-zinc-400" : "text-muted-foreground"}`}>
                    {contact.name} ({contact.email})
                  </p>
                  <p className={`mt-2 text-xs ${contact.status === "RESOLVED" ? "text-zinc-400" : "text-muted"}`}>
                ...
                <div className="flex gap-2">
                  ...
                  {contact.status !== "RESOLVED" && (
                    <select
                      ...
                      disabled={processing}
                      className="rounded-md border border-border bg-card px-2 py-1.5 text-xs font-medium text-muted-foreground focus:border-border-strong focus:outline-none focus:ring-1 focus:ring-accent-mint disabled:opacity-50"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-border bg-card p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-foreground">お問い合わせの詳細</h2>
              <Button
                variant="secondary"
                size="sm"
                rounded="md"
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-sm text-foreground">{selectedContact.title}</p>
              </div>

              <div>
                <h3 className="text-sm font-medium text-muted-foreground">お名前</h3>
                <p className="mt-1 text-sm text-muted-foreground">{selectedContact.name}</p>
              </div>

              <div>
                <h3 className="text-sm font-medium text-muted-foreground">メールアドレス</h3>
                <p className="mt-1 text-sm text-muted-foreground">{selectedContact.email}</p>
              ...
              <div>
                <h3 className="text-sm font-medium text-muted-foreground">お問い合わせ内容</h3>
              ...
              {selectedContact.admin_note && (
                <div>
                  <h3 className="text-sm font-medium text-muted-foreground">管理者メモ</h3>
                  <p className="mt-1 text-sm text-muted-foreground whitespace-pre-wrap">
                    {selectedContact.admin_note}
                  </p>
                </div>
              ...
              {selectedContact.status !== "RESOLVED" && (
                <div className="flex gap-2 pt-4">
                  <select
                    ...
                    disabled={processing}
                    className="flex-1 rounded-md border border-border bg-card px-4 py-2 text-sm font-medium text-muted-foreground focus:border-border-strong focus:outline-none focus:ring-1 focus:ring-accent-mint disabled:opacity-50"
                  >
                    <option value="PENDING">未処理</option>
                    <option value="PROCESSING">処理中</option>
                    <option value="RESOLVED">対応済</option>
